...
local_dir    <- "raw"
data_source <- paste0(local_dir, "/pages_2019")
...
bylaws_url <- paste0("https://ak-static.cms.nba.com/wp-content/uploads/sites/",
                     "4/2019/09/NBA-Constitution-By-Laws-September-2019-1.pdf")
...
map2(bylaws, seq(length(bylaws)), function(bylaws, page){
  ...
  temp_file <- paste0(data_source, "/bylaws_2019_page_", page, ".txt")
...
fileConn <- file(paste0(local_dir, "/bylaws_2019.txt"))
writeLines(bylaws, fileConn)
close(fileConn)